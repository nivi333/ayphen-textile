  ...
  const columns = [
    {
      ...
      render: (record: ProductionOrder) => (
        <div>
          ...
          {record.customer && (
            <div style={{ fontSize: '12px', color: '#666' }}>Customer: {record.customer.name}</div>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (record: ProductionOrder) => (
        <Space>
          ...
          />
          <Button type='text' icon={<DeleteOutlined />} size='small' danger title='Delete Order' />
        </Space>
      ),
    },
  ];
  ...
  const handleWorkOrderSubmit = async (values: any) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL || '/api/v1'}/production/work-orders`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
        ...
        <div className='production-header'>
          ...
          <div className='production-header-actions'>
            <GradientButton size='small' onClick={handleCreateOrder}>
              <Typography.Text>Create Production Order</Typography.Text>
            </GradientButton>
          </div>
        </div>
              ...
              <Col xs={24} sm={12} md={6}>
                <Card>
                  <Statistic
                    title='Completed'
                    value={
                      (summary.statusBreakdown.READY || 0) +
                      (summary.statusBreakdown.SHIPPED || 0) +
                      (summary.statusBreakdown.DELIVERED || 0)
                    }
              ...
              <Col xs={24} sm={12} md={6}>
                <Card>
                  <Statistic
                    title='High Priority'
                    value={
                      summary.priorityBreakdown.HIGH || 0 + summary.priorityBreakdown.URGENT || 0
                    }
          ...
          <Form form={form} layout='vertical' onFinish={handleFormSubmit}>
            ...
            <Form.Item style={{ textAlign: 'right', marginBottom: 0 }}>
              ...
              <GradientButton type='primary' htmlType='submit'>
                <Typography.Text>{editingOrder ? 'Update Order' : 'Create Order'}</Typography.Text>
              </GradientButton>
            </Form.Item>
          </Form>
        </Modal>
        ...
        >
          <Form form={workOrderForm} layout='vertical' onFinish={handleWorkOrderSubmit}>
            ...
            <Form.Item style={{ textAlign: 'right', marginBottom: 0 }}>
              ...
              <GradientButton type='primary' htmlType='submit'>
                <Typography.Text>Create Work Order</Typography.Text>
              </GradientButton>
            </Form.Item>
          </Form>
        </Modal>
                ...
                <Row gutter={[16, 16]}>
                  <Col span={12}>
                    <Card size='small' title='Product Information'>
                      ...
                      <p>
                        <strong>SKU:</strong> {selectedOrder.productSku}
                      </p>
                      <p>
                        <strong>Category:</strong> {selectedOrder.category}
                      </p>
                      {selectedOrder.fiberType && (
                        <p>
                          <strong>Fiber Type:</strong> {selectedOrder.fiberType}
                        </p>
                      )}
                      {selectedOrder.yarnCount && (
                        <p>
                          <strong>Yarn Count:</strong> {selectedOrder.yarnCount}
                        </p>
                      )}
                      {selectedOrder.gsm && (
                        <p>
                          <strong>GSM:</strong> {selectedOrder.gsm}
                        </p>
                      )}
                    </Card>
                  </Col>
                  <Col span={12}>
                    <Card size='small' title='Production Progress'>
                      <p>
                        <strong>Ordered:</strong> {selectedOrder.orderedQuantity}
                      </p>
                      <p>
                        <strong>Produced:</strong> {selectedOrder.producedQuantity}
                      </p>
                      <p>
                        <strong>Rejected:</strong> {selectedOrder.rejectedQuantity}
                      </p>
                      <Progress
                        percent={Math.round(
                          (selectedOrder.producedQuantity / selectedOrder.orderedQuantity) * 100
                        )}
                        size='small'
                      />
                    </Card>
