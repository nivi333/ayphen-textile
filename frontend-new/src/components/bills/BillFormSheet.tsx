...
export function BillFormSheet({ open, onClose, initialData, isLoading }: BillFormSheetProps) {
    ...
    if (open && locations.length > 0) {
      if (initialData) {
        ...
        if (!initialData) {
          ...
          // Only reset if we haven't initialized yet (using a ref or check if form is dirty)
          // But since we want to keep the reset for other fields but PRESERVE the location if already set:
          // Actually, simply removing the full form.reset for new entries in this effect might be safer
          // if we only want to set defaults.
          // However, the original code used reset to set ALL defaults.
          // Let's use reset options or check if values are effectively default.

          // Better approach: Only reset if the form is not dirty, or just set specific defaults.
          // The issue is likely that fetchData finishes, triggers this effect, and re-resets.

          // If we are creating new (no initialData), and we haven't touched the form:
