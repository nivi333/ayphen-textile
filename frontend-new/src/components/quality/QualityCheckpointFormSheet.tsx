...
export function QualityCheckpointFormSheet({ open, onOpenChange, onSuccess, checkpoint }: QualityCheckpointFormSheetProps) {
  ...
  useEffect(() => {
    if (open && checkpoint) {
      form.reset({
        ...
        inspectionDate: checkpoint.inspectionDate ? new Date(checkpoint.inspectionDate) : new Date(),
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className='w-[720px] sm:max-w-[720px] overflow-y-auto'>
        <SheetHeader className='flex flex-row items-center justify-between space-y-0 pb-4'>
          <SheetTitle>{isEditing ? 'Edit Quality Checkpoint' : 'New Quality Checkpoint'}</SheetTitle>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
            <div className='space-y-4'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <FormField control={form.control} name='checkpointType' render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
                        <SelectTrigger><SelectValue placeholder='Select checkpoint type' /></SelectTrigger>
                      </FormControl>
                      <SelectContent>
                ...
                <FormField control={form.control} name='checkpointName' render={({ field }) => (
                  <FormItem>
                    <FormLabel required>Checkpoint Name</FormLabel>
                    <FormControl><Input placeholder='Enter checkpoint name' {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <FormField control={form.control} name='inspectorName' render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder='Enter inspector name' {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              ...
              <div className='grid grid-cols-2 gap-4'>
                <FormField control={form.control} name='batchNumber' render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder='Enter batch number' {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              ...
              <div className='grid grid-cols-3 gap-4'>
                ...
                <FormField control={form.control} name='testedQuantity' render={({ field }) => (
                  <FormItem>
              ...
              <FormField control={form.control} name='notes' render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Textarea placeholder='Enter notes' className='min-h-[80px]' {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
